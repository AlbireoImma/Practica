### get_proc_ip_server.ps1 - Francisco Abarca - 06-02-2019
## Script el cual retrae los procesos ejecutandose en los equipos designados por el archivo IP_SERVER.txt
...
$ruta = ".\log\PROCESOS\procesos_SERVER_$(Get-Date -Uformat "%d%m%Y%H%M").csv" # Ruta para el archivo generado
...
foreach ($server in $servers) { # Recorremos todas las IP del listado $servers
    ...
    } catch { # Si hubo un error en la ejecución del bloque anterior
        $linea = $DNS[$contador] + "," + $server + ",,,,,,,,1" # Generamos una linea con codigo de error 1
        $linea >> $ruta # Escribimos la linea al archivo generado en la variable $ruta
    }
